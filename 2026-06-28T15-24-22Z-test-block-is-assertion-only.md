...
**Why:** The test block should read as a pure statement of the asserted truth — a single relation among named values — not a computation mixed with a check. Naming every operand (in the context, where the controls are declared) documents what each value is, gives each an inspection point, and cleanly separates the control and actuation phases from the observation. An assertion with an inlined constructor call buries a value inside the predicate and forces the reader to parse it inside-out. It is the same standard everywhere a value is inlined into a call: bind the value to a name, then use the name. Here it applies to *both* operands, not only the actuation's result.

**How to apply:** In the enclosing `context`, declare the controls, bind the actuation to an explaining variable, and bind any compared-against/expected value to its own explaining variable. The `test` block contains exactly one `assert`, comparing those variables — no value-producing method call inside it. The comparison operator (`==`, etc.) is the assertion's predicate, not an inlined value-producing call, so it stays. Related: the test-structure rule and the `control_` test-variable prefix rule.
